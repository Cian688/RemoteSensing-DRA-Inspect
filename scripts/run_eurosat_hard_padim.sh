...
OUTPUT_DIR="outputs/remote_sensing/results/eurosat_hard_padim"
LOG_DIR="logs/eurosat_hard_padim"
SEED=42
...
run_protocol_block() {
  ...
  for degradation in "${DEGRADATIONS[@]}"; do
    local log_path="${LOG_DIR}/${normal_tag}_${hard_protocol}_${degradation}_padim.log"
    echo "==> GPU${gpu_id} | ${hard_protocol} | ${degradation} | PaDiM"
    python scripts/run_padim_remote_sensing.py \
      --config "${CONFIG_PATH}" \
      --dataset eurosat \
      --protocol hard \
...
wait "${PID1}" "${PID2}" "${PID3}"
echo "All EuroSAT-Hard PaDiM runs finished."
